...
# WE MAY HAVE AN ISSUE LATER AND IT MAY HAVE TO DO WITH WARPING
...
tm_shape(central_california) +
  tm_polygons() +  # Add borders to the bounding box
  tm_shape(locations) +
  tm_raster() +
  tm_shape(central_california) +
  tm_borders()
...
First I am just going to make a function that will map an area based on temperature and depth.

# Try to put them both together and it worked!

```{r}
area_map_fun <- function(min_temp, max_temp, min_depth, max_depth) {
...
```{r}
area_map_fun(-Inf, Inf, -Inf, Inf)
```

# Try adding species name
...
```{r}
create_suitable_area_map <- function(min_temp, max_temp, min_depth, max_depth, species_name) {
...
# Convert logical values (TRUE/FALSE) to integers (1/0)
depth_range <- st_apply(suitable_depth, c("x", "y"), function(x) as.integer(x))

combo_range <- suitable_sst * suitable_depth
  
# Step 3: Convert suitable area to a stars object
suitable_area_stars <- st_as_stars(combo_range)
...
```{r}
create_suitable_area_map <- function(min_temp, max_temp, min_depth, max_depth, species_name, sst_raster, depth_raster, shapefile) {
  
  # Step 1: Ensure the CRS consistency
  if (st_crs(sst_raster) != st_crs(shapefile)) {
    shapefile <- st_transform(shapefile, st_crs(sst_raster))
  }
...
min_temp <- 8  # Minimum sea surface temperature (°C)
max_temp <- 18  # Maximum sea surface temperature (°C)
