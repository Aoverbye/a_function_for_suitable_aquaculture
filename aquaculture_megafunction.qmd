...
area_map_fun <- function(min_temp, max_temp, min_depth, max_depth, species_name) {
  ...
  suitable_test <- combo_range * region_raster
   ...
   # Step 6: Use Zonal Statistics to Aggregate the Suitable Areas
  # Here, we use zonal algebra to sum the suitable habitat area within each region
  suitable_area <- zonal(x = suitable_test, 
                         z = region_raster, # Raster representing zones
                         fun = 'sum',       # Sum of suitable habitat pixels
                         na.rm = TRUE)
...
# TRYING TO RE DO IT BUT THIS TIME USING THE terra PACKAGE

```{r}
# If sst_clean and depth_clean are stars objects, convert them to terra rasters
sst_raster <- rast(sst_clean)  # Convert SST data to terra raster
depth_raster <- rast(depth_clean)  # Convert Depth data to terra raster
...
```{r}
# Reclassify SST (Temperature) based on the desired range (11°C to 30°C)
suitable_sst <- (sst_raster >= 11 & sst_raster <= 30) * 1  # 1 for suitable, 0 for unsuitable

# Reclassify Depth based on the desired range (-70m to 0m)
suitable_depth <- (depth_raster >= -70 & depth_raster <= 0) * 1  # 1 for suitable, 0 for unsuitable

# Plot the reclassified SST and Depth maps
plot(suitable_sst)
plot(suitable_depth)
...
```{r}
# Combine the suitable SST and Depth rasters (both need to be 1 for suitable areas)
suitable_locations <- suitable_sst * suitable_depth

# Plot the combined map of suitable locations
plot(suitable_locations)
...
```{r}
 # Step 4: Rasterize the region shapefile (wc_regions_clean) to match the suitable area raster
  # Rasterize the region shapefile based on the "rgn" field, aligning it with the suitable area raster
  region_raster <- rasterize(wc_regions_clean, suitable_locations, field = "rgn", fun = "count")
  
  # Step 5: Multiply the suitable area raster by the region raster to isolate suitable regions
  suitable_test <- suitable_locations * region_raster
```

```{r}
# Step 6: Use Zonal Statistics to Aggregate the Suitable Areas
  # Here, we use zonal algebra to sum the suitable habitat area within each region
  region_sums2 <- zonal(x = suitable_test, 
                         z = region_raster, # Raster representing zones
                         fun = 'sum',       # Sum of suitable habitat pixels
                         na.rm = TRUE)
```
...
```{r}
agg_raster <- region_raster

# Assign the summarized values (from zonal statistics) back to the raster
agg_raster[] <- region_sums2$sum[match(region_raster[], region_sums2$rgn)]

# Plot the aggregated raster
plot(agg_raster, main = "Aggregated Suitable Habitat Area by Region")
```
...
```{r}
# Step 1: Rasterize the wc_regions_clean shapefile to match the suitable_locations raster
region_raster <- st_rasterize(wc_regions_clean, suitable_locations, field = "rgn", fun = "count")
...
```{r}
suitable_by_region <- rast(suitable_by_region)
region_raster <- rast(region_raster)
```
...
# Step 6: Use Zonal Statistics to Aggregate the Suitable Areas
  # Here, we use zonal algebra to sum the suitable habitat area within each region
  region_sums2 <- zonal(x = suitable_by_region, 
                         z = region_raster, # Raster representing zones
                         fun = 'sum',       # Sum of suitable habitat pixels
                         na.rm = TRUE)
```
...
```{r}
region_sums2 <- rename(region_sums2, area = area_m2_area_m2)
```

```{r}
print(paste0("Oyster suitable area in ", region_sums2$rgn[3], " is ", round(region_sums2$area[3], digits = 2), 'km'))
```
...
```{r}
# Function to create a map showing suitable habitat for a species based on temperature and depth criteria
test_map_fun <- function(min_temp, max_temp, min_depth, max_depth, species_name) {
  ...
  # Step 5: Multiply the suitable area raster by the region raster to isolate suitable regions
  suitable_region <- combo_range * region_raster
...
```{r}
# Assuming region_sums2 has region sums, and region_sums is a stars object

# Step 1: Extract the region sums and reshape region_sums2
region_sums2 <- region_sums2 %>%
  rename(area_km2 = area) %>%
  select(rgn_id_rgn_id, area_km2, average_annual_sst_2008.tif, NA._NA.)

# Step 2: Extract the spatial data from the 'stars' object (region_sums)
# We need to extract the values corresponding to the regions

# Assuming that 'rgn_id' is the region identifier and corresponds to regions in region_sums
region_sums_values <- as.data.frame(region_sums) %>%
  select(rgn_id, average_annual_sst_2008.tif)

# Step 3: Merge region_sums2 with the region sums values
combined_data <- left_join(region_sums2, region_sums_values, by = "rgn_id_rgn_id")

# Step 4: Create a new raster (if necessary) with the region sums and suitable area data
# This step is needed for the heatmap creation
# Assuming that the region boundaries are already represented in the region_sums stars object

# Extract the suitable area as a new layer from region_sums (assuming it's stored in one of the attributes)
region_raster <- region_sums["area_km2"]  # Change this to the correct attribute

# Plot the region sums as a heatmap (coloring by suitable area)
tm_shape(region_raster) +
  tm_raster(palette = "YlGnBu", title = "Suitable Habitat Area") +
  tm_shape(combined_data) +
  tm_borders(lwd = 2) +
  tm_layout(main.title = "Suitable Area by Region", legend.position = c(0.7, 0.1))
```
